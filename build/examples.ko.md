...
Apps SDK 애플리케이션 구축을 위한 엔드투엔드 예제입니다.

## 개요
Pizzaz 데모 앱은 여러 UI 컴포넌트를 번들로 제공하여 전체 도구 표면 영역을 엔드투엔드로 확인할 수 있도록 합니다. 다음 섹션에서는 이러한 도구를 구동하는 MCP 서버와 컴포넌트 구현에 대해 설명합니다.

"Pizzaz" 데모 앱과 기타 예제는 [GitHub의 예제 저장소](https://github.com/openai/openai-apps-sdk-examples)에서 확인할 수 있습니다.

이러한 예제를 자신만의 앱을 구성할 때 청사진으로 활용하세요.

## MCP 소스
이 TypeScript 서버는 사전 구축된 UI 리소스와 데이터를 공유하는 여러 도구를 등록하는 방법을 보여줍니다. 각 리소스 호출은 Skybridge HTML 셸을 반환하며, 모든 도구는 ChatGPT가 어떤 컴포넌트를 렌더링할지 알 수 있도록 일치하는 메타데이터로 응답합니다.
...
![Pizzaz map 컴포넌트의 스크린샷](https://developers.openai.com/images/apps-sdk/pizzaz-map.png)

map 컴포넌트는 상태를 ChatGPT와 동기화하는 React + Mapbox 클라이언트입니다. 마커 상호작용, inspector 라우팅, 전체 화면 처리를 렌더링하여 더 복잡하고 상태를 가진 컴포넌트 예제를 학습할 수 있습니다.
...
![Pizzaz carousel 컴포넌트의 스크린샷](https://developers.openai.com/images/apps-sdk/pizzaz-carousel.png)

이 carousel은 가벼운 갤러리 뷰를 구축하는 방법을 보여줍니다. 터치 친화적인 스크롤을 위해 embla-carousel을 활용하고 버튼 상태를 연결하여 서버 왕복 없이도 컴포넌트가 반응성을 유지하도록 합니다.
...
![Pizzaz list 컴포넌트의 스크린샷](https://developers.openai.com/images/apps-sdk/pizzaz-list.png)

이 list 레이아웃은 채팅으로 시작된 일정이나 보고서에 삽입할 수 있는 형태를 반영합니다. 히어로 요약과 스크롤 가능한 순위 목록의 균형을 맞춰 컴포넌트 내에서 더 밀도 높은 정보 계층 구조를 실험할 수 있습니다.
...
## Pizzaz Video 소스
video 컴포넌트는 재생을 추적하고, 컨트롤을 오버레이하며, 전체 화면 변경에 반응하는 스크립트형 플레이어를 래핑합니다. ChatGPT 컨테이너 API와 통합해야 하는 미디어 중심 경험의 참고 자료로 활용하세요.
